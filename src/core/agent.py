...
from src.actions import get_all_available_action_data
...
from src.core.task import get_stringified_all_available_task_name_description_pairs, \
...
from src.domain.models.task_spec.generic_task_state import GenericTaskState
...
class Agent:
    ...
    def start(self):
        if self._monitor_task is None or self._monitor_task.done():
            ...
            self._monitor_task.add_done_callback(self._handle_monitor_crash)
    ...
    def _handle_monitor_crash(self, task: asyncio.Task):
        ...
        try:
            ...
            print(f"\n[FATAL ERROR] Agent Monitor crashed silently: {e}")
            import traceback
            traceback.print_exc()
            # Force the pause event to set so the main thread doesn't hang forever
            if self._pause_event:
                self._pause_event.set()
