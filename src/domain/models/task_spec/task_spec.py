...
from src.core.task import get_all_task_spec_paths
from src.core.task.task_instance import TaskInstance
from src.utils.prompt_utils import load_prompt_template, replace_all_keys_in_prompt_template
from src.domain.models.task_spec.information_spec import InformationSpec, parse_information_spec
...
class TaskSpec:
    ...
    def __init__(
        ...
        self.output_information_spec = \
            {k: parse_information_spec(v)
             for k, v in output_information_spec.items()}
    ...
    def _load_task_spec_dict(task_spec_path, task_spec_str, task_specs_root_path):
        if ((task_spec_str is None and task_spec_path is None)
            ...
            raise ValueError(
                'Exactly one of task_spec_str or task_spec_path should be provided.')
